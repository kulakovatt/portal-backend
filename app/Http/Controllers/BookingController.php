...
class BookingController extends Controller
{
    ...
    public function ticket_reserv(BookingRequest $request){
        ...
        $date = $tours->where('id', $request->id_tour)->get('date_flight')[0]->date_flight;
        $date_flight = Carbon::parse($date);
        ...
        $number_queue = $booking->where('id_tour', $request->id_tour)->count() + 1;
        ...
        Mail::send( 'mail', ['firstname' => $firstname, 'lastname' => $lastname,
            'tour' => $name_tour, 'duration' => $duration, 'residence' => $residence,
            'date_flight' => $date_flight->locale('ru_RU')->isoFormat('D MMMM YYYY'), 'price' => $price, 'queue' => $number_queue], function ($message){
